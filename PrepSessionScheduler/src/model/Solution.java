...
public class Solution implements Comparable<Solution>{
	 ...
	 */
	public void findAllMembers(ArrayList<Student> students, int blockSize){
		...
		for (Student student:students){
			...
			for (Session session:sessions){
				if (student.hasBlockOpen(session.timeSlot, blockSize)) {
					session.add(student);
	...
	public boolean isSame(Solution sol){
		...
		for (Session s:this.sessions){
			...
			if (thisTimeSlot.compareTo(otherTimeSlot)!=0) return false;
		}
		return true;
	}
	...
	public boolean isSimilar(Solution sol){
		...
		for (Session s:this.sessions){
			...
			else if (Math.abs(thisTimeSlot.getTime()-otherTimeSlot.getTime())>2) return false;
